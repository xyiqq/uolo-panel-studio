...
export function auditElectrical(design,assembly,net,budgets=[]) {
  ...
  const liveWires=(net?.wires||[]).filter(w=>!(design.disconnected||[]).includes(w.id));
  for(const mod of design.modules||[]){
    ...
    for(const [ch,id] of Object.entries(mod.channels||{})){
      ...
      if(!c||!loads.length||loads.some(l=>!Number.isFinite(l.watts))||!(c.pf>0)) {known=false;add('CHANNEL_LOAD_UNKNOWN','pending',mod.id,`${mod.id} 通道 ${ch}（${id}）负荷或功率因数未完整录入`);continue;}
      const watts=loads.reduce((sum,l)=>sum+l.watts,0),amps=watts/((c.voltage===380?Math.sqrt(3)*380:220)*c.pf);total+=amps;
      if(Number.isFinite(p.channelAmps)) {if(amps>p.channelAmps)add('CHANNEL_OVERCURRENT','error',mod.id,`${mod.id} 通道 ${ch}（${id}）估算 ${amps.toFixed(2)} A，超过通道额定 ${p.channelAmps} A`);}
      else add('CHANNEL_RATING_UNKNOWN','pending',mod.id,`${mod.id} 通道 ${ch}（${id}）适用负载类型的额定电流待核`);
      if(Number.isFinite(p.channelWatts)&&watts>p.channelWatts)add('CHANNEL_OVERPOWER','error',mod.id,`${mod.id} 通道 ${ch}（${id}）${watts} W 超过额定 ${p.channelWatts} W`);
    }
    const rating=p.totalAmps??p.amps;
    if(Object.values(mod.channels||{}).some(Boolean)) {
      if(Number.isFinite(rating)&&known&&total>rating)add('MODULE_TOTAL_OVERCURRENT','error',mod.id,`${mod.id} 已分配通道合计 ${total.toFixed(2)} A，超过模块总额定 ${rating} A`);
      if(!Number.isFinite(rating))add('MODULE_TOTAL_UNKNOWN','pending',mod.id,`${mod.id} 模块总额定电流待核`);
    }
    if(isMainsFedModule(p)){
      ...
      for(const feed of feeds){
        ...
        if(Number.isFinite(allowed)&&Number.isFinite(amps)&&amps>allowed)add('MODULE_PROTECTION_RATING','error',mod.id,`${mod.id} 上游 ${feed.breakerId} 为 ${amps} A，超过已录入配合额定 ${allowed} A`);
        else if(!Number.isFinite(allowed))add('MODULE_PROTECTION_UNKNOWN','pending',mod.id,`${mod.id} 与 ${feed.breakerId} 的上游保护配合额定待核`);
      ...
      if(p.powerInput==='LN'&&p.id!=='crestron-din-8sw8-i'&&!liveWires.some(w=>w.to===`${mod.id}:N_IN`))add('MODULE_NEUTRAL_MISSING','error',mod.id,`${mod.id} 需要 N 输入，当前没有有效零线接入`);
    }
