import {createStandaloneHtml} from './standalone-html.js';
import {readSitePhoto} from './site-photo.js';
import {psuConnectionFields,modulePowerDescription} from './psu-connections.js';
import {psuConnectionType,savePsuConnections} from '../core/psu-connections.js';
import {resolveDesignLink} from '../core/design-link.js';
import { createIcons, icons } from "lucide";
import { clearDevices } from "../core/clear-devices.js";
import { mountOptionalFeatures } from './optional-features.js';
import { openDuctCabinetDialog } from './duct-cabinet-dialog.js';
...
import {resolveModuleFeedBindings,isMainsFedModule} from '../core/module-feeds.js';
...
<main class="body-grid">
  <aside class="left">
    <section class="side-section"><div class="section-head"><h2>\u9879\u76EE\u914D\u7F6E</h2>${We("settings","settings-2","项目信息与设计条件")}</div>
...
function gX(){
  ...
  EA&&!Xe.wires.some(a=>a.id===EA)&&(EA=null)
    ...
    <button class="btn primary" id="cab-save">\u751f\u6210\u5e76\u5207\u6362</button> <button class="btn" id="cab-duct">按优诺线槽标准…</button>`);
  R("#cab-duct").onclick=()=>openDuctCabinetDialog({open:MA,close:()=>R("#modal").close(),commit:de,toast:pe});
...
function VX(a){const wireRange=Number.isFinite(a.minWire)&&Number.isFinite(a.maxWire)?`${a.minWire}\u2013${a.maxWire} mm\xB2`:'\u5F85\u6838';return`<div class="spec-row"><span>\u8BA2\u5355\u53F7</span><strong>${it(gA(a))}</strong></div><div class="spec-row"><span>\u54C1\u724C / \u578B\u53F7</span><strong>${it(a.brand)}<br>${it(a.name)}</strong></div>
  ...
  <p class="footnote" style="margin-top:10px">${it(a.availability)}</p>`}function As(){let a=es(),t=kt.nodes.find(f=>f.id===je)||a&&kt.nodes.find(f=>f.id===a.id);if(EA){let f=Xe.wires.find(c=>c.id===EA),u=Je.filter(c=>c.wire===f.id||c.text.includes(f.from)),p=fr(f.section,f.method||"B1",f.ambient||40,f.bunched||1,f.loaded||(Y.supply==="three"?3:2));return R("#inspector").innerHTML=`<section class="side-section"><div class="section-head"><h2>\u5BFC\u7EBF\u5C5E\u6027</h2><span class="tag">${f.conductor}</span></div>
    ...
    <section class="side-section"><h3>\u8FDE\u63A5\u8FB9\u754C</h3><p class="condition">RCBO \u540E\u7684 N \u53EA\u8FDB\u5165\u672C\u56DE\u8DEF\u8D1F\u8F7D\uFF1BPE \u4E0D\u7ECF\u8FC7\u4EFB\u4F55\u65AD\u8DEF\u5668\u3001\u6F0F\u4FDD\u6216\u63A5\u89E6\u5668\u3002SPD \u4E3A\u5E76\u8054\u8BBE\u5907\uFF0C\u4EE5 PE \u4E3A\u6CC4\u653E\u7AEF\uFF0C\u4E0D\u6DFB\u52A0\u865A\u5047\u7684\u4E32\u8054\u8D1F\u8F7D\u51FA\u7EBF\u3002</p></section>`,R("#wire-section").onchange=c=>de(v=>v.wireOverrides[f.id]=+c.target.value),R("#wire-reset").onclick=()=>de(c=>delete c.wireOverrides[f.id]),R("#wire-toggle").onclick=()=>de(c=>{c.disconnected=f.connected?[...c.disconnected,f.id]:c.disconnected.filter(v=>v!==f.id)}),Ye()}if(!a){let f=t?.product;return R("#inspector").innerHTML=`<section class="side-section"><div class="section-head"><h2>${it(t?.label||"\u9879\u76EE\u6761\u4EF6")}</h2><span class="tag">${t?.id||"PROJECT"}</span></div>
      ...
      ${modulePowerDescription(t.product)?`<p class="condition">${it(modulePowerDescription(t.product))}</p>`:''}
      ${!isMainsFedModule(t.product)?'':Bt(t.product.loadPowerInput==='LN'?"灯具／设备负载的支路空开":"对应支路空开（一对一）",`<select id="insp-mod-protect">${re(breakerOptions(t.id),t.module?.protectId||"")}</select>`)}
      ...
      <button class="btn" id="project-settings" style="width:100%;margin-top:12px">${Ut("settings-2")}项目信息与设计条件</button></section>
  ...
  <section class="side-section full-only">
    ...
    ${s.slice(0,8).map(f=>`<div class="spec-row"><span>${it(f.name)}</span><strong>${f.enabled?f.watts+" W":"\u4E0D\u8BA1\u5165"}<br><small>${it(f.powerCell||"\u81EA\u5B9A\u4E49")}</small></strong></div>`).join("")}
  ...
  if(isSimpleMode(Y)){
    ...
    if(Ja==="channels"){
      ...
      t=$a(["产品","订单号","数量","宽度","模数"],[...e.values()].map(({product:r,count:s})=>`<tr><td><button class="link" data-product="${it(r.id)}">${it(r.name)}</button></td><td>${it(gA(r))}</td><td>${s}</td><td>${r.width??"—"} mm</td><td>${(r.modules||0)*s} M</td></tr>`));
    }
  ...
  let a=Y.loads.filter(e=>e.enabled).reduce((e,r)=>e+r.watts,0),t="";if(Ja==="circuits")t=$a(["\u56DE\u8DEF","\u6846\u67B6\u5206\u533A","\u7535\u6E90","\u8D1F\u8377","cos\u03C6","Ib / In / Iz","\u51FA\u7BB1\u7535\u7F06","\u72B6\u6001"],Y.circuits.map(e=>{let r=kA(Y,e);return`<tr class="${es()?.id===e.id?"selected":""}">
        ...
        <td class="source-cell">${it(e.powerCell||"\u81EA\u5B9A\u4E49")}</td><td>${it(e.zone)}</td><td>${it(e.name)}</td>
    ...
    <div class="subnav"><button data-audit="issues" class="${zr==="issues"?"active":""}">\u95EE\u9898\u6E05\u5355 ${Je.length}</button><button data-audit="framework" class="${zr==="framework"?"active":""}">\u6846\u67B6\u6620\u5C04 21</button><button data-audit="sources" class="${zr==="sources"?"active":""}">\u7D20\u6750\u4E0E\u4F9D\u636E</button></div>${a}`,document.querySelectorAll("[data-audit]").forEach(t=>t.onclick=()=>{zr=t.dataset.audit,MX(),Ye()}),document.querySelectorAll("[data-asset]").forEach(t=>t.onclick=()=>MA("\u6765\u6E90\u8BC1\u636E",`<img class="modal-image" src="${To[t.dataset.asset]}" alt="\u6765\u6E90\u9875"><p class="tiny">\u539F\u59CB\u8D44\u6599\u53CA\u63D0\u53D6\u8BB0\u5F55\u5747\u4FDD\u5B58\u5728 V2 \u5DE5\u7A0B\u76EE\u5F55\u4E2D\u3002</p>`)),document.querySelectorAll("[data-select]").forEach(t=>t.onclick=()=>{VA(t.dataset.select),innerWidth<=1e3&&R(".right").classList.add("open")}),R("#audit-json").onclick=()=>nn("\u6761\u4EF6\u6821\u6838\u62A5\u544A.json",JSON.stringify({name:Y.name,generated:new Date().toISOString(),scope:"\u975E\u65BD\u5DE5\u5408\u683C\u7ED3\u8BBA",issues:Je},null,2))}function jU(a){if(a==="\u603B\u5F00")return"4P / 2P \u8FDB\u7EBF\u603B\u5F00\uFF0C\u540C\u65F6\u65AD\u5F00\u76F8\u7EBF\u548C N\uFF1B\u4FDD\u7559\u5FC5\u8981 SPD \u53CA\u5176\u540E\u5907\u4FDD\u62A4\uFF0C\u4E0D\u53E6\u8BBE\u63A7\u5236\u652F\u8DEF\u3002";if(a==="\u667A\u63A7\u7CFB\u7EDF")return"\u5F31\u7535\u3001\u7F51\u7EDC\u4E0E\u5B89\u9632\u72EC\u7ACB\u56DE\u8DEF\uFF1B\u5206\u533A\u4EC5\u4E3A\u903B\u8F91\u5206\u7EC4\uFF0C\u4E0D\u91CD\u590D\u8BBE\u7F6E\u5206\u533A\u603B\u5F00\u3002";let t=Y.circuits.filter(e=>e.path.includes(a));return t.length?t.map(e=>e.id+" "+e.name).join("\uFF1B"):"\u6309\u7236\u5206\u533A\u53CA\u72EC\u7ACB\u8BBE\u5907\u56DE\u8DEF\u5C55\u5F00\uFF1B\u672A\u628A\u680F\u76EE\u76F4\u63A5\u7B49\u540C\u4E8E\u4E00\u4E2A\u65AD\u8DEF\u5668\u3002"}function yX(){let sitePhoto=Y.sitePhoto||null,photoRead=0;MA("方案设置",`
    <h3>项目基本信息</h3>
    ${Bt("方案名称",`<input id="settings-name" value="${it(Y.name)}" maxlength="80">`)}
    ${Bt("公司名称",`<input id="settings-company" value="${it(Y.companyName||'')}" maxlength="120" placeholder="填写交付公司名称（选填）">`)}
    ${Bt("项目地址",`<textarea id="settings-address" rows="2" maxlength="240" placeholder="填写项目所在地、楼栋及房号（选填）">${it(Y.projectAddress||'')}</textarea>`)}
    <p class="tiny">公司名称和项目地址随当前方案保存，并显示在导出 PDF 的第一页。</p>
    <h3>配电箱现场实拍照片</h3>
    <p class="tiny">上传装配完成后的照片，PDF 会在三维方案图后加入独立实拍页。支持 JPG、PNG、WebP，最大 12 MB；图片仅保存在本机方案中。</p>
    <input id="settings-site-photo" type="file" accept="image/jpeg,image/png,image/webp" aria-label="上传配电箱现场实拍照片">
    <div id="settings-photo-preview"></div><p id="settings-photo-status" class="tiny" role="status"></p>
    <button id="settings-photo-remove" type="button" class="btn">移除照片</button>
    <h3>界面与设计条件</h3>
    <p class="tiny">控制箱外电源示意块及其引入线的显示，截图和 PDF 三维方案图同步；不改变实际接线。</p>
    ...
    <button id="settings-save" class="btn primary">应用</button>`);
    const paintPhoto=()=>{const root=R("#settings-photo-preview");root.replaceChildren();if(sitePhoto?.dataUrl&&/^data:image\/(jpeg|png|webp);base64,[A-Za-z0-9+/=]+$/.test(sitePhoto.dataUrl)){const img=document.createElement("img");img.src=sitePhoto.dataUrl;img.alt="现场实拍照片预览";img.style.cssText="max-width:100%;max-height:180px;object-fit:contain;margin:10px 0";root.append(img);}R("#settings-photo-remove").disabled=!sitePhoto;R("#settings-photo-status").textContent=sitePhoto?`已选择：${sitePhoto.name||"现场照片"}，点击应用保存`:"尚未上传现场照片";};
    paintPhoto();
    R("#settings-site-photo").onchange=async event=>{const file=event.target.files[0];if(!file)return;const sequence=++photoRead,save=R("#settings-save"),status=R("#settings-photo-status");save.disabled=true;status.textContent="正在处理照片…";try{const photo=await readSitePhoto(file);if(sequence!==photoRead||!event.target.isConnected||!R("#modal").open)return;sitePhoto=photo;paintPhoto();}catch(error){if(sequence===photoRead)status.textContent=error.message;}finally{if(sequence===photoRead)save.disabled=false;}};
    R("#settings-photo-remove").onclick=()=>{photoRead++;sitePhoto=null;R("#settings-site-photo").value="";R("#settings-save").disabled=false;paintPhoto();};
    R("#settings-save").onclick=()=>{
      const mode=R("#settings-ui-mode").value;
      const showSupplyIllustration=false;
      const name=R("#settings-name").value.trim()||Y.name;
      const companyName=R("#settings-company").value.trim();
      const projectAddress=R("#settings-address").value.trim();
      const mainAmps=+R("#settings-main").value;
      const nBar=R("#settings-nbar")?.value||'bottom';
      if(isSimpleMode(Y)||mode==="simple"){
        const layoutChanged=mainAmps!==Y.mainAmps||nBar!==(Y.nBarPosition||'bottom');
        R("#modal").close();
        de(r=>{Object.assign(r,{name,companyName,projectAddress,sitePhoto,showSupplyIllustration}); r.nBarPosition=nBar; if(mainAmps!==r.mainAmps){r.mainAmps=mainAmps;r.mainProductId=null} setUiMode(r,mode)},layoutChanged);
      ...
      let e={name,companyName,projectAddress,sitePhoto,showSupplyIllustration,mainAmps,demand:a,sourceIsc:t,earthing:R("#settings-earth").value,feedMethod:R("#settings-method").value,feedAmbient:+R("#settings-temp").value,feedBunched:+R("#settings-bunch").value,nBarPosition:nBar};
      const wiringChanged=['mainAmps','demand','sourceIsc','earthing','feedMethod','feedAmbient','feedBunched','nBarPosition'].some(key=>e[key]!==Y[key]);
      R("#modal").close(),de(r=>{e.mainAmps!==r.mainAmps&&(r.mainProductId=null),Object.assign(r,e),setUiMode(r,mode)},wiringChanged)
    }}
...
function openModuleDialog(product, existing){
  ...
  const hasPsuOutputs=!!psuConnectionType(product);
  MA(editing?"编辑模块 "+id:product.outletCount?"装入PDU":isTerm?"装入菲尼克斯端子":product.networkPorts?(product.poe?"装入PoE网关":"装入交换机"):"装入智能模块",`
    ...
    ${modulePowerDescription(product)?`<p class="condition" id="mod-power-description">${it(modulePowerDescription(product))}</p>`:''}
    ...
    ${!isMainsFedModule(product)||product.networkPorts||product.outletCount?"":Bt(product.loadPowerInput==='LN'?"灯具／设备负载的支路空开":"对应支路空开（一对一）",`<select id="mod-protect">${re(breakerOptions(id),protectId||"")}</select>`)}
    <div style="${hasPsuOutputs||product.networkPorts||product.outletCount?"display:none":""}" class="${simple||isTerm?"full-only":""}">
      <div class="field-grid">
        ${Bt("总线",`<select id="mod-bus">${re(busOpts,busId||"")}</select>`)}
        ${Bt(product.loadPowerInput==="LN"?"灯具／设备负载馈电方式":"馈电方式",`<select id="mod-feed">${re(feedOpts,feed)}</select>`)}
      </div>
    ...
    ${psuConnectionFields(Y,id,product,pid=>Me(Y,pid),it)}
    ...
    <p class="lib-preview">${product.outletCount?"PDU用支架固定，模位为安装占位；所接设备文字用于标识，不生成未经核实的电气连线。":product.networkPorts?(product.powerInput==="ac-inlet"?"网络端口不生成 PoE 或市电接线；本机为内置交流电源，请预留后部电源线及接地空间。":(product.poe?"本机支持PoE供电；当前模型显示接口与备注，不模拟PoE链路。另行预留54V DC / 2.4A外置适配器空间。":"网络端口不生成 PoE 或市电接线；请另行预留外置 5V DC / 1A 适配器空间。")):isTerm?"灯线先压到端子 FIELD 侧，PANEL 侧跳到继电器。零线用蓝色端子。":!isMainsFedModule(product)?"模块控制电源请在对应直流电源的下端出线中选择；总线接口用于通信。":simple?"支路空开对应模块的负载侧，向灯具／设备供电；模块本身的直流供电另行选择。总空开不参与分配。":"多路模块只占一个导轨位；可同时备注通道并关联回路。"}</p>
  ...
  R("#mod-save").onclick=()=>{
    ...
    try{
      ...
      const outputTargets=[...document.querySelectorAll('[data-psu-target]:checked')].map(el=>el.dataset.psuTarget);
      if(hasPsuOutputs){
        const draft=structuredClone(Y);
        const index=draft.modules.findIndex(m=>m.id===id);
        index>=0?draft.modules[index]=mod:draft.modules.push(mod);
        savePsuConnections(draft,id,product,outputTargets);
      }
      de(d=>{
        ensureModuleArrays(d);
        const i=d.modules.findIndex(m=>m.id===id);
        i>=0?d.modules[i]=mod:d.modules.push(mod);
        if(hasPsuOutputs) savePsuConnections(d,id,product,outputTargets);
        else for(const b of d.buses){
          if(b.id!==existing?.busId||b.id===mod.busId) continue;
          b.psuModuleIds=(b.psuModuleIds||[]).filter(x=>x!==id);
          b.deviceModuleIds=(b.deviceModuleIds||[]).filter(x=>x!==id);
        }
        if(mod.busId&&!hasPsuOutputs){
          let bus=d.buses.find(b=>b.id===mod.busId);
          if(bus){
            if(product.kind==="psu") bus.psuModuleIds.push(id);
            else if(!bus.deviceModuleIds.includes(id)) bus.deviceModuleIds.push(id);
          }
        }
...
function YU(a,t){let r=kt.nodes.find(s=>s.id===a);if(!r||!canPlaceFootprint(Y,r.product,kt.box,kt.occupied,t,a))return pe("\u76EE\u6807\u4F4D\u7F6E\u8D85\u51FA\u5BFC\u8F68\u6216\u5DF2\u6709\u5668\u4EF6");setNodePosition(a,t),VA(a)}function nn(a,t,e="application/json"){let r=URL.createObjectURL(new Blob([t],{type:e})),s=document.createElement("a");s.href=r,s.download=a,s.click(),setTimeout(()=>URL.revokeObjectURL(r),15e3)}function IX(a,t){let e=r=>{let s=String(r??"");return/^[=+\-@\t\r]/.test(s)&&(s="'"+s),'"'+s.replaceAll('"','""')+'"'};nn(a,"\uFEFF"+t.map(r=>r.map(e).join(",")).join(`\r
`),"text/csv;charset=utf-8")}function zX(){IX("\u5168\u5C4B\u56DE\u8DEF\u8BA1\u7B97.csv",[["\u56DE\u8DEF","\u540D\u79F0","\u6E90\u6846\u67B6","\u7535\u538BV","\u76F8\u4F4D","\u5DF2\u77E5\u8D1F\u8377W","\u529F\u7387\u56E0\u6570","Ib_A","\u5668\u4EF6\u8BA2\u5355\u53F7","In_A","\u5BBD\u5EA6mm","Iz_A","\u7EBF\u7F06","\u538B\u964D\u521D\u4F30\u767E\u5206\u6BD4","\u9700\u6838\u4E8B\u9879"],...Y.circuits.map(a=>{let t=kA(Y,a);return[a.id,a.name,a.path,a.voltage,a.phase,t.p,a.pf,t.ib.toFixed(3),gA(t.product),t.product.amps,t.product.width,t.iz.toFixed(2),t.cable,t.drop.toFixed(2),Je.filter(e=>e.circuit===a.id).map(e=>e.text).join("\uFF1B")]})])}function wX(){const net=buildDeliveryNet(Y,kt,Xe);IX("全柜端子接线表.csv",deliveryWireRows(net,deliveryLabels(Y,net)))}function standaloneHtmlString(design=Y){return createStandaloneHtml(design)}function _U(){window.__V5_DOCS__.prepare(async()=>{try{nn(Y.name+'-独立方案.html',await standaloneHtmlString(),'text/html;charset=utf-8')}catch(e){pe(e.message)}})}function $U(){MA(isSimpleMode(Y)?"导出方案":"导出 V4 方案",`<p>${it(Y.name)} · ${Y.circuits.length} 回路 · ${(Y.modules||[]).length} 模块。导出保留当前备注与共用保护，不能作为工程合格声明。</p>
...
G1&&pe(G1);addEventListener("storage",a=>{a.key===hX&&a.newValue!=null&&a.newValue!==JSON.stringify(Y)&&pe("该方案已在另一个标签页修改；继续在此编辑会覆盖对方的修改，建议刷新页面")});

function focusDesignLink(){
  const link=resolveDesignLink(location.href,Y);
  document.querySelector('#design-link-notice')?.remove();
  if(link.status==='none')return;
  if(link.status==='resolved') {if(link.circuitId)VA(link.circuitId);return;}
  const notice=document.createElement('div');notice.id='design-link-notice';notice.setAttribute('role','alert');
  notice.style.cssText='position:fixed;bottom:20px;left:50%;transform:translateX(-50%);z-index:50;background:#352719;color:#ffe0ae;border:1px solid #ac8552;padding:12px;border-radius:8px;max-width:90vw;font-size:13px';
  const text=document.createElement('span');text.textContent=link.message;notice.append(text);
  const button=document.createElement('button');button.className='btn';button.textContent='导入对应方案';button.onclick=()=>R('#file').click();notice.append(button);
  const dismiss=document.createElement('button');dismiss.className='btn';dismiss.textContent='关闭';dismiss.onclick=()=>notice.remove();notice.append(dismiss);document.body.append(notice);
}
const importHandler=R('#file').onchange;
R('#file').onchange=async event=>{await importHandler(event);focusDesignLink()};
focusDesignLink();
